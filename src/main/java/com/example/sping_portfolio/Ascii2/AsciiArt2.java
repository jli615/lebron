package com.example.sping_portfolio.Ascii2;
...
public class AsciiArt2 {
    ...
    public static char getGreyScaleChars(int n) {
        return GRAYSCALE_CHARS.charAt(n);
    }
    /**
    ...
    public static void main(String[] args) {
        Picture picture = new Picture("src/main/resources/static/images/curry.png");
        ...
        for (int i = 0; i < rows; i+=2) {
            for (int j = 0; j < cols; j+=2) {
                /* Search for greyscale value in GRAYSCALE_VALUES array*/
                int grayscaleIndex=0;
                double mean = (image[i][j] + image[i][j+1]+
                        image[i+1][j] +image[i+1][j+1])/4;

                while (grayscaleIndex < GRAYSCALE_VALUES.length)
                {
                    if (GRAYSCALE_VALUES[grayscaleIndex] < mean)
                    {
                        grayscaleIndex++;
                    }
                    else
                ...
                System.out.print(getGreyScaleChars(grayscaleIndex)+ " ");
            }
            System.out.println();
        }
    }
